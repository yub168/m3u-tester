...
def creatIptvTestList(sourcelist=None,startNum=None,endNum=None,codeTo4Str=None):
    ...
    startNum = startNum if startNum else 0
    endNum = endNum if endNum else 1200
    codeTo4Str = codeTo4Str if codeTo4Str else 1
    if not sourcelist:
      sourcelist={
          #'101.229.227.246:7777':'http://101.229.227.246:7777/tsfile/live/{}_1.m3u8?key=txiptv&playlive=1&authid=0',#速度有点卡 300-600K 已无法用
          ...
          #'113.57.93.165:9900':'http://113.57.93.165:9900/tsfile/live/{}_1.m3u8?key=txiptv&playlive=1&authid=0', # 速度不好200K，且台少 央卫视编码同上 放弃
          ...
          #'223.151.49.74:59901':'http://223.151.49.74:59901/tsfile/live/{}_1.m3u8?key=txiptv&playlive=1&authid=0',# 速度较好600-1000以上且稳定 卫视台少，多湖南省台 失效
          #'223.159.8.218:8099':'http://223.159.8.218:8099/tsfile/live/{}_1.m3u8?key=txiptv&playlive=0&authid=0', #速度较好400-800 卫视台少，多湖南省台 失效
          ...
          #'222.134.245.16:9901':'http://222.134.245.16:9901/tsfile/live/{}_1.m3u8?key=txiptv&playlive=1&authid=0', # 没有结果 测速很好 但放不动
    ...
    if codeTo4Str:
       ...
       numList.extend(list(range(1000,1200)))
    else:
       numList=list(range(200))
...
def autoipTvTest(sourcelist=None,startNum=None,endNum=None,codeTo4Str=None,minSpeed=None,minHeight=None,testTime=None):
  ...
  for group,channels in dicts.items():
    ...
    if results:
      ...
      test.writeTestInfo(f'============{group} 测试完成,共获取 {len(results)} 个地址,用时 ：{round((time.time()-startTime)/60,3)}==================')
    else:
       test.writeTestInfo(f'============{group} 测试完成,共获取 0 个地址,用时 ：{round((time.time()-startTime)/60,3)}==================')
